...
class MagnetometerCalculator():
    ...
    def _loadConfig(self,filename):
        ...
        self.consider_freq      = Dict['consider_freq']  #in Hz
    ...
    def _transfer_based_on_voltages(self,freq,voltage):
        ...
        response = self._Response_funtion(np.abs(freq))
        response = self._conjugate(freq,response)
    ...
    def calculateVoltages(self, **kwargs):
        ...
        # get frequencies
        freq                        = fftpack.fftfreq(num_samples, sample_step*1e-9)
        # signal transfer
